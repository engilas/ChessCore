...
internal class Program
{
        ...
        while (true)
            try
            {
                ...
                else
                {
                    ...
                    if (move.StartsWith("edit")) continue;
                    if (move == "hint") continue;
                    if (move == "bk") continue;
                    if (move == "undo")
                    {
                        engine.Undo();
                    ...
                    if (move == "easy") continue;
                    if (move.StartsWith("accepted")) continue;
                    if (move.StartsWith("rejected")) continue;
                    if (move.StartsWith("variant")) continue;
                    if (move == "random") continue;
                    if (move == "force") continue;
                    if (move == "go") continue;
                    if (move == "playother")
                    {
                        if (engine.WhoseMove == ChessPieceColor.White)
                            engine.HumanPlayer = ChessPieceColor.Black;
                        else if (engine.WhoseMove == ChessPieceColor.Black) engine.HumanPlayer = ChessPieceColor.White;
                    ...
                    if (move == "white")
                    {
                        ...
                        if (engine.WhoseMove != engine.HumanPlayer) MakeEngineMove(engine);
                        continue;
                    }
                    ...
                    if (move.StartsWith("level")) continue;
                    if (move.StartsWith("st")) continue;
                    if (move.StartsWith("sd")) continue;
                    if (move.StartsWith("time")) continue;
                    if (move.StartsWith("otim")) continue;
                    if (move.StartsWith("otim")) continue;
                    if (move == "?") continue;
                    if (move.StartsWith("ping"))
                    {
                        if (move.IndexOf(" ") > 0)
                        {
                            var pong = move.Substring(move.IndexOf(" "), move.Length - move.IndexOf(" "));
                    ...
                    if (move.StartsWith("result")) continue;

                    if (move.StartsWith("setboard"))
                    {
                        if (move.IndexOf(" ") > 0)
                        {
                            var fen = move.Substring(move.IndexOf(" "), move.Length - move.IndexOf(" ")).Trim();
                    ...
                    if (move.StartsWith("setboard")) continue;
    ...
    private static void MakeEngineMove(Engine engine)
    {
        ...
        {
            if (sourceColumn != destinationColumn) tmp += GetColumnFromInt(sourceColumn + 1);
        }

        if (lastMove.TakenPiece.PieceType != ChessPieceType.None) tmp += "x";
        ...
        if (lastMove.PawnPromotedTo == ChessPieceType.Queen)
            ...
            tmp += "=K";
        else if (lastMove.PawnPromotedTo == ChessPieceType.Bishop) tmp += "=B";
    ...
    private static string GetPgnMove(ChessPieceType pieceType)
    {
        ...
        if (pieceType == ChessPieceType.Bishop)
            ...
            move += "Q";
        else if (pieceType == ChessPieceType.Rook) move += "R";
    ...
    private static byte GetColumn(string move)
    {
        if (move != null)
            if (move.Length == 2)
            {
                var col = move.Substring(0, 1).ToLower();
    ...
    private static void DrawBoard(Engine engine)
    {
        ...
        for (byte i = 0; i < 64; i++)
        {
            ...
            var PieceType = engine.GetPieceTypeAt(i);
            var PieceColor = engine.GetPieceColorAt(i);
            string str;
            ...
            if (PieceColor == ChessPieceColor.Black) str = str.ToLower();
