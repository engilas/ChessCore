...
namespace ChessEngine.Engine
{
    internal class Book
    {
        ...
        {
            var openingBook = new List<OpeningMove>();
            string moveLine;
            ...
            foreach (var m in moves) openingMove.Moves.Add(new MoveContent(m));
            openingMove = new OpeningMove();
        ...
        internal static int ValidateOpeningBook(List<OpeningMove> openingBook)
        {
            ...
            foreach (var moves in openingBook)
            foreach (var move in moves.Moves)
                if (!IsValidMove(move.MovingPiecePrimary.SrcPosition, move.MovingPiecePrimary.DstPosition,
                    moves.StartingFEN))
                    failed++;
        ...
        {
            var chessBoard = new Board(fen);
            PieceValidMoves.GenerateValidMoves(chessBoard);


            if (chessBoard.Squares == null) return false;

            if (chessBoard.Squares[srcPos].Piece == null) return false;

            foreach (var bs in chessBoard.Squares[srcPos].Piece.ValidMoves)
                if (bs == dstPos)
                    return true;

            if (dstPos == chessBoard.EnPassantPosition) return true;
