namespace ChessEngine.Engine
{
    internal static class PieceSquareTable
    {
        ...
        private static readonly short[] KingEndGameTable =
        {
        ...
        private static readonly short[] KingMiddleGameTable =
        {
        ...
        private static readonly short[] PawnTableEndGame =
        {
            9000, 9000, 9000, 9000, 9000, 9000, 9000, 9000,
            ...
            byte position, bool endGame)
        {
            switch (PieceColor)
            {
                case ChessPieceColor.White:
                    ...
                    {
                        if (endGame) return PawnTableEndGame[position];

                        return PawnTable[position];
                    }

                    if (PieceType == ChessPieceType.Knight) return KnightTable[position];
                    if (PieceType == ChessPieceType.Bishop) return BishopTable[position];
                    if (PieceType == ChessPieceType.Rook) return RookTable[position];
                    if (PieceType == ChessPieceType.Queen) return QueenTable[position];
                    if (PieceType == ChessPieceType.King)
                    {
                        if (endGame) return KingEndGameTable[position];
                ...
                case ChessPieceColor.Black:
                    ...
                    var index = (byte) ((byte) (position + 56) - (byte) ((byte) (position / 8) * 16));

                    if (PieceType == ChessPieceType.Pawn)
                    {
                        if (endGame) return PawnTableEndGame[index];

                        return PawnTable[index];
                    }

                    if (PieceType == ChessPieceType.Knight) return KnightTable[index];
                    if (PieceType == ChessPieceType.Bishop) return BishopTable[index];
                    if (PieceType == ChessPieceType.Rook) return RookTable[index];
                    if (PieceType == ChessPieceType.Queen) return QueenTable[index];
                    if (PieceType == ChessPieceType.King)
                    {
                        if (endGame) return KingEndGameTable[index];
